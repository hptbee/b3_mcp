...
This document is the source of truth for the detailed roadmap.

README.md should remain concise and traditional.
...
B3 must work without internet access.

No required external APIs, cloud services, hosted vector databases, SaaS authentication, telemetry, or paid services.

### 2. Free-by-default

Core functionality must be free to run locally.

Paid/proprietary/cloud integrations are allowed only as optional plugins, disabled by default.

### 3. MCP runtime stays thin

The MCP runtime is a protocol/tool adapter only.
...
### 4. Storage owns persistence

SQLite schema, migrations, transactions, repositories, persistence internals, and SQLite-backed adapters belong in `b3-storage`.

### 5. Indexer owns indexing

Parsing, file discovery, hashing, incremental indexing, watcher behavior, parser worker behavior, and parse failure reporting belong in `b3-indexer`.

### 6. Query owns intelligence

Graph traversal, ranking, context packing, impact analysis, dependency tracing, cycle detection, and query trace belong in `b3-query`.

### 7. Control server is an adapter

The control server exposes localhost HTTP/SSE APIs.

It should not own storage internals, indexing logic, query ranking, or MCP protocol logic.

### 8. Web UI talks only to local control server

The web UI must not call cloud services.

It should call the local control server only.

### 9. Graph first, semantic second

AST, graph, and FTS are core.

Embeddings and semantic search are optional future enhancements and must remain secondary to graph/AST/FTS intelligence.

### 10. Benchmark before optimization

No speculative optimization.

Measure first, optimize measured bottlenecks only, and report before/after observations.

### 11. Refactor only after verified milestones

Prefer small targeted refactors.

Avoid architecture rewrites and premature generic abstractions.

### 12. Language support must be layered

Do not implement 20 languages in one phase.

Use a language backend architecture with tree-sitter and LSP capabilities.

### 13. LSP complements tree-sitter

Tree-sitter is used for fast local indexing.

LSP is used later for semantic references, definitions, diagnostics, rename, and safe symbolic editing.

LSP must use local/free language servers by default.

### 14. Project model supports standalone and grouped projects

Some projects are independent.

Some projects belong to a system group.

B3 must support both:

- standalone project
- project group / workspace

The default storage model remains repo-local:

```text
1 project = 1 .b3/b3.db
```
...
Optimized for future C# / TypeScript / React / Angular workflows.
  ...
  "projects": [
    {
      "id": "fnb-backend",
      "name": "F&B Backend",
      "path": "D:\\Project\\FnbBackend",
      "database": "D:\\Project\\FnbBackend\\.b3\\b3.db",
      "tags": ["csharp", "api", "fnb"]
    },
    {
      "id": "fnb-frontend",
      "name": "F&B Frontend",
      "path": "D:\\Project\\FnbFrontend",
      "database": "D:\\Project\\FnbFrontend\\.b3\\b3.db",
      "tags": ["react", "angular", "frontend", "fnb"]
    }
  ],
  "groups": [
    {
      "id": "fnb-system",
      "name": "F&B System",
      "description": "Backend, frontend, workers, POS, and runtime infrastructure.",
      "project_ids": [
        "fnb-backend",
        "fnb-frontend",
        "order-worker",
...
## Phase 9.2 — F&B / Web App Priority Support
...
- DTOs
- tests
- ASP.NET routes where possible

### React / Angular Detection
...
- client events
- groups
- frontend SignalR client handlers
...
- `SendAsync` event names
- frontend `.on` handlers
...
- Docker extraction quality
- SignalR extraction quality
- WPF extraction quality
- false positives
- false negatives
...
- clean `LanguageBackend` abstractions
- remove duplicated AST utilities
- normalize language-specific tests
- stabilize plugin boundaries
- simplify LSP/tree-sitter integration points
...
- language pack indexing speed
- LSP process reuse
- parser worker batching
- multi-language graph traversal
- large-repo query latency
- memory usage
...
- semantic search
- semantic + graph reranking
- semantic fallback
- semantic duplicate dedup
...
- Codex config helper
- Claude config helper if applicable
- doctor command
- uninstall command
- optional Tauri app
- release packaging
...
Benchmark fixtures must remain:

- local
- deterministic
- small enough to commit
- free from network dependency
- free from cloud/API calls
...
- Phase 3 to Phase 7
- future Phase 9.x for multi-language graph expansion

B3 intentionally differs:

- stronger offline/free governance
- Rust-native architecture
- explicit control server/UI/debugging boundaries
- query trace and benchmark-first roadmap
...
- token-saving retrieval
- indexed context instead of repeated grep/read
- context pack strategy
- savings ledger concept
...
- Phase 5.1
- Phase 5.2
- Phase 8.5 as adjacent command compaction layer

B3 intentionally differs:

- persistent graph + query engine + UI
- command output compaction
- future session compaction
- strict offline/free requirement
...
- command output compaction
- git/test/build/lint output summarization
- local proxy/token-saving ideas
...
- RTK-like ideas are one layer only
- B3 also owns code graph, UI, MCP tools, query trace, and storage
- no command execution in compaction tool
...
- graph intelligence remains core
- session memory must be local-only
...
- Rust-native storage/query/control design
- explicit offline/free requirement
- graph explorer + query trace UI

### CodeGraph

B3 learns:

- broad multi-language graph MCP
- framework-aware route detection
- with/without MCP benchmarks
- agent steering
...
- query trace and control UI
- LSP + tree-sitter hybrid plan
- stricter offline/free defaults
...
- Rust-native local architecture
- SQLite/FTS code graph
- MCP runtime stays thin
- no required cloud/service layer
...
- LSP backend
- IDE-grade semantic operations
- find definition/references/implementations
- symbolic editing
- rename/refactor
- mode/profile system
...
- no required paid JetBrains plugin
- LSP must be local/free by default
- tree-sitter graph remains core
- LSP complements graph, does not replace it
...
- impact analysis
- context retrieval
- developer-oriented code navigation
