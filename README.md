...
- Symbol search and code search
- Callers/callees query
- Related symbol discovery
- Impact analysis
- Dependency tracing and cycle detection
...
Rust currently has the best language support. Broader real-world app-stack intelligence for C#, TypeScript, React, Angular, Node.js, Docker, Kafka, RabbitMQ, SignalR, WPF, Three.js, and other project types is planned for Phase 9.x.
...
- C#, TypeScript, React, Angular, Node.js, Docker, Kafka, ksqlDB, RabbitMQ, SignalR, WPF, Three.js, and other app-stack support are planned for Phase 9.x.
...
- `crates/b3-core` — shared contracts, config, events, and common DTOs
- `crates/b3-storage` — SQLite persistence, migrations, storage repositories, and SQLite-backed adapters
- `crates/b3-indexer` — indexing, parsing, file watcher, parser isolation, and parser worker
- `crates/b3-query` — search, graph traversal, ranking, context packs, impact analysis, query trace, and centrality usage
- `crates/b3-mcp-runtime` — thin MCP stdio runtime and MCP tool exposure
- `crates/b3-control` — localhost HTTP/SSE control server
- `crates/b3-bench` — local benchmark harness and deterministic benchmark fixtures
- `crates/b3-compaction` — deterministic local command output compaction
- `apps/web-ui` — local Next.js web UI
- `benchmarks/fixtures` — small deterministic benchmark repositories
...
The compaction layer keeps important information such as errors, warnings, failed tests, compiler diagnostics, changed files, diff summaries, non-zero exit status, container/service status, search result summaries, and truncation metadata.

It never executes commands, shells out, calls an LLM, calls an external API, uploads command output, or emits telemetry.
...
The benchmark runner measures local behavior such as cold startup, MCP tools/list latency, query latency, graph query latency, indexing speed, parser worker request latency, and command compaction latency.
...
For production-like usage on C#, Node.js, React, Angular, Docker, Kafka, RabbitMQ, SignalR, WPF, Three.js, and other application stacks, wait for the Phase 9.x language and domain intelligence work.
